...
- Meal-time reminder feature is confirmed wanted but not yet designed/scoped technically (push notification via the PWA?).
...
- Food-vs-blood-sugar correlation: a real analytics feature, bigger than the other two — needs its own design pass (what counts as "close enough" in time to link a reading to a meal, how to summarize per-food patterns) before implementation. Treat as a future milestone, not immediate scope.
