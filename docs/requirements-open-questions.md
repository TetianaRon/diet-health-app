...
- Meal-time reminder feature is confirmed wanted but not yet designed/scoped technically — **next design topic queued up**, see `docs/build-log.md`'s 2026-09-07 "designed, not yet built" entry. Open question going in: real OS-level push notification (needs a backend we don't have) vs. a lighter in-app signal.
...
- ~~Food-vs-blood-sugar correlation~~ — **rescoped and fully designed 2026-09-07, no longer an open question.** Deliberately cut down from a real analytics feature to a manual-review one, per the developer/mom's own suggestion: a "meals before this reading" expandable list on the Blood Sugar screen (pure timestamp lookup, no correlation logic) plus a manual watch/avoid flag on both Ingredients and Dishes. Full design (including the ingredient↔dish flag interaction rules) is recorded in `docs/build-log.md`'s 2026-09-07 entry — ready to implement, not yet built.
