...
- **Nutrition data is bundled-first, API-fallback:** a curated starter set of common Ukrainian staple foods (plus a static Glycemic Index reference table — no free API provides GI) ships with the app and is browsable/usable everywhere ingredients are picked from (Foods list, dish composition, meal logging) without first being individually saved to the Ingredients sheet — see Technical Decisions below. Only genuinely new foods trigger a live lookup.
...
- **Language compatibility:** USDA is English-only, but the UI is Ukrainian-only, and mom doesn't speak English. She only ever types a Ukrainian food name; `NameEn` (used for the API query) is resolved automatically — from the bundle when it's a known food, or via a free translation API (MyMemory, no key needed) when it's not. She's never asked to *supply* an English name — but the resolved `NameEn` is shown afterward as a subtle secondary label wherever foods are listed, as a fallback cross-check (not something she needs to read).
- **Blood sugar units:** mmol/L (Canada/Ukraine standard)
- **New product validation:** the bundle itself needs no approval step — it's already reviewed, curated data, usable immediately. Saving to the Ingredients tab is only needed for a genuinely new food (App suggests, from USDA or manual entry → mom approves → saved) or to mark a bundle item as a favorite (which saves it automatically, as a side effect of favoriting)
